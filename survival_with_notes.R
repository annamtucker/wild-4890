...
c = 0.1   # daily censorship probability
...
# visualize fates over time
require(tidyverse)

pop %>% 
  data.frame() %>%
  mutate(ind = c(1:nrow(pop))) %>% 
  gather(time, state, 1:10) %>% 
  mutate(time = as.numeric(substr(time, 2, nchar(time)))) %>% 
  ggplot(aes(x = time, y = ind)) +
  geom_tile(aes(fill = state), col = "gray70") +
  xlab("Day") +
  ylab("Individual") +
  scale_y_continuous(breaks = seq(1, nrow(pop), 1)) +
  scale_x_continuous(breaks= seq(1, 10, 1)) +
  scale_fill_manual(values = c("palegreen4", "darkgreen", "black", "gray70"),
                    labels = c("Alive", "Censored", "Dead", ""), name= "") +
  theme_bw() +
...
# need to calculate the number alive and number dead in each group at each time period
daily$n.alive = daily$n.risk - daily$n.censored - daily$n.deaths

test = rbind(daily[10,], daily[10,])
test$group = c("A", "B")

test2 = as.matrix(test[,c(3,9)])
...
# how does survival vary between sites?
